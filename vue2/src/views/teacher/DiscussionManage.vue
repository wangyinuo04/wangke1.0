    ...
    async saveTopic() {
      ...
      try {
        ...
        if (response.success) {
          ...
          // 修复：立即更新本地数据，而不是重新加载所有数据
          if (response.data) {
            // 将新话题添加到 topics 数组的开头
            const newTopic = {
              ...response.data,
              topicId: response.data.topicId || response.data.id, // 确保有 topicId
              authorName: this.teacherName, // 添加作者名称
              role: 'teacher', // 设置角色
              isTop: this.form.isTop,
              author: this.teacherName,
              title: this.form.topicTitle,
              content: this.form.topicContent,
            ...
            // 如果是置顶话题，放在最前面，否则放在非置顶话题的最前面
            if (this.form.isTop) {
              this.topics.unshift(newTopic)
            } else {
              // 找到最后一个置顶话题的位置
              const lastTopIndex = this.topics.findIndex(topic => topic.topOrder === 0 || !topic.isTop)
              if (lastTopIndex === -1) {
                this.topics.push(newTopic)
              } else {
                this.topics.splice(lastTopIndex, 0, newTopic)
              }
            }
          } else {
            // 如果后端没有返回完整数据，重新加载列表
